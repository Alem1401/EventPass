...
export class VenueService {
  ...
  getVenueTypes(){
    return this.http.get<venueTypeDto[]>("https://localhost:7231/api/VenueTypes");
  }

  getVenueById(id : number){
    return this.http.get<VenueResponseDto>(`https://localhost:7231/api/Venue/${id}`);
  }
  ...
  getSectionsByVenue(venueId : number){
    return this.http.get<responseSectionDto[]>(`https://localhost:7231/api/Section/venue/${venueId}`);
